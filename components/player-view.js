...
module.exports = class Player extends React.Component {
  ...
  onLoaded(duration) {
    ...
    emitter.emit('load', {value: duration});
  }

  componentDidUpdate(prevProps, prevState) {
    if (!this.props.player) {
      console.log('componentDidUpdate NO player: ', this.props, 'prevProps', prevProps);
      return;
    }
  ...
  attachVideoListeners() {
    console.log('attachVideoListeners props:', this.props);
    if (!this.props.src) return;

    if (this.props.player === 'youtube') {
      console.log('youtube', emitter, this);
      emitter.emit('init', {
        onLoaded: this.onLoaded.bind(this)
      });
    } else {
