...
function launchVideo(opts) {
  ...
  windowUtils.whenReady(() => {
    ...
    windowUtils.send('set-video', {
      ...
      loaded: false,// TODO: still needed? from rebase
      progress: 0,  // TODO: still needed? from rebase
      playing: false// TODO: still needed? from rebase
    });
    if (!opts.src) {
      opts.getUrlFn(id, function(err, streamUrl) {
        ...
        windowUtils.send('set-video', {
          src: streamUrl,
          error: Boolean(err),
          strings: getLocaleStrings(opts.domain) // TODO: do we need to send this down twice?
        });
      }, opts.time);
    }
  });
