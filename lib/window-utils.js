...
const DEFAULT_DIMENSIONS = {
  height: 180,
  width: 320,
  minimizedHeight: system.platform === 'linux' ? 50 : 40
};
...
// Opens the video in the original tab, if it is still open, else in a new tab.
function sendToTab(panel, opts) {
  const pageUrl = getPageUrl(opts.domain, opts.id, opts.time);

  if (pageUrl) {
    const tabs = require('sdk/tabs');
    let targetTab = null;
    const pageVideoId = getVideoId(pageUrl);
    // try to find the original tab that had the video
    for (let tab of tabs) { // eslint-disable-line prefer-const
      if (tab.id === opts.tabId && pageVideoId && getVideoId(tab.url) === pageVideoId) {
        targetTab = tab;
        break;
      }
    }
    ...
    if (targetTab === null) {
      // try to find any tab that has the video open currently
      for (let tab of tabs) { // eslint-disable-line prefer-const
        if (pageVideoId && getVideoId(tab.url) === pageVideoId) {
          targetTab = tab;
          break;
        }
      }
