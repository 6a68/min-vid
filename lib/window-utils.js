...
function create() {
  ...
  mvWindow = window.open(self.data.url('default.html'), 'minvid',
                         'chrome,dialog=no,width=320,height=180,titlebar=yes,alwaysRaised');
...
function makeDraggable() {
  // just show the titlebar on hover, that's it.
  // the dragging happens that way.
  // append a special stylesheet.

  setTimeout(() => {
    let titlebar = mvWindow.document.getElementById('titlebar');
    titlebar.setAttribute('hidden', true);
    mvWindow.addEventListener('mouseenter', () => { titlebar.removeAttribute('hidden') });
    mvWindow.addEventListener('mouseleave', () => { titlebar.setAttribute('hidden', true) });
  }, 5000); // try waiting a while. will the titlebar appear?
}
